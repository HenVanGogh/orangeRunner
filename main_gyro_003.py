...
if __name__ == '__main__':
    ...
    def executeBase(base):
        ...
        data = [ -base[0][0]-  (126.8095 / math.sqrt(2)) , base[0][1] , -base[0][2]+   (126.8095 / math.sqrt(2)) ,
                base[1][0]-   (126.8095 / math.sqrt(2)),   base[1][1] ,    base[1][2]-  (126.8095 / math.sqrt(2)) ,
                base[2][0]-  (126.8095 / math.sqrt(2)) ,  base[2][1] ,  base[2][2]+  (126.8095 / math.sqrt(2)) ,
                -base[3][0]-   (126.8095 / math.sqrt(2)) , base[3][1] , -base[3][2]-   (126.8095 / math.sqrt(2))]
        data.append(int(round(time.time() * 1000)) - lastTime)
        data.append(gyro.receivePacket)
        csv.registerData(data)
    ...
    def left_front():
        ...
        for step in range(0 , speed * 16):
            ...
            move3(-pose[2][1][step]+ xBias , pose[2][0][step]+ yBias , -pose[2][2][step] - zBias , 0)
            move4( pose[1][1][step]+ xBias , pose[1][0][step]+ yBias , pose[1][2][step] +  zBias , 0)
            data = [pose[0][1][step] + xBias , pose[0][0][step] + yBias , pose[0][2][step] - zBias,
                 -pose[3][1][step]+ xBias , pose[3][0][step]+ yBias , -pose[3][2][step] + zBias ,
                 -pose[2][1][step]+ xBias , pose[2][0][step]+ yBias , -pose[2][2][step] - zBias ,
                 pose[1][1][step] + xBias, pose[1][0][step] + yBias, pose[1][2][step] + zBias]
            data.append(int(round(time.time() * 1000)) - lastTime)
            data.append(gyro.receivePacket)
            csv.registerData(data)
